...
export default function SurveyScreen() {
  ...
  async function finish(answer: Parameters<typeof auth.saveSurvey>[0]) {
    ...
    setLoading(true);
    setError(null);
    const err = await auth.saveSurvey(answer);
    if (err) {
      setLoading(false);
      setError(i18n.t('survey.error'));
      return;
    }
  ...
  return (
    ...
    >
      <TouchableOpacity
        ...
        testID="survey-skip"
        disabled={loading}
        onPress={() => void finish({ skipped: true })}
      >
        <Text style={[styles.skip, { color: colors.textSecondary }]}>{i18n.t('survey.skip')}</Text>
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  skip: { textAlign: 'right', padding: 10, fontWeight: '600' },
