...
jest.mock('@/contexts/AuthContext', () => ({ useAuth: () => ({ beginEmailUpgrade: mockBegin, signIn: mockSignIn, signInWithApple: jest.fn() }) }));
...
jest.mock('@/components/account', () => { const { TouchableOpacity: Button, Text: Label } = require('react-native'); return { AccountForm: ({ onSubmit, mode }: { onSubmit: (email: string, password: string) => void; mode: string }) => <Button testID="form" onPress={() => onSubmit('a@b.com', 'Aa!123456')}><Label>{mode}</Label></Button> }; });
...
describe('AccountScreen route contracts', () => {
  beforeEach(() => { jest.clearAllMocks(); mockParams = {}; mockPendingAction = null; });
