...
describe('SurveyScreen route contracts', () => { beforeEach(() => jest.clearAllMocks()); it('saves typed answers then replays the pending action', async () => { const { getByTestId } = render(<SurveyScreen />); fireEvent.press(getByTestId('submit')); await waitFor(() => expect(mockSave).toHaveBeenCalledWith({ acquisitionSource: 'app_store' })); expect(mockFinish).toHaveBeenCalled(); }); it('skips with the explicit skipped payload', async () => { const { getByTestId } = render(<SurveyScreen />); fireEvent.press(getByTestId('survey-skip')); await waitFor(() => expect(mockSave).toHaveBeenCalledWith({ skipped: true })); }); });
